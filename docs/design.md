...
**skit** 是一个用 **Go** 编写的高性能 Skill 管理 CLI 工具，定位为 **“Skill 生态的 go-style 工具链”**：启动快、行为可预测、锁文件可复现，并通过 `.agents/skills` active 目录提供稳定安装入口。

**使命**：
- 遵守 `agentskills.io` 定义的 `SKILL.md` 格式；
- 支持主流 source syntax 和 project/global scope；
- 参考 `clawhub` 的 registry、版本、元数据、安全扫描与 HTTP API 设计；
- 引入可复现 lockfile、source provider、系统依赖诊断和 `metadata.skit` 扩展协议；
...
- 自动执行系统包管理器安装依赖；
- 自建中心化 registry；
- 管理多个 Agent 私有 target 目录或 copy 同步状态；
...
**skit 设计**：
- `.agents/skills/skit.lock` 记录完整递归依赖树中每个 Skill 的精确版本、提交哈希和校验和。
...
**保持 `SKILL.md` 作为运行时入口，同时把包管理、依赖和诊断元数据逐步分层**。`name` 和 `description` 必须留在 `SKILL.md` 中；更重的包管理信息优先放入 `metadata.skit`。
...
| `.agents/skills/skit.lock` | 递归依赖树的精确版本锁定 | skit CLI | `install` / `update` 时自动生成 |
...
3. `metadata.clawdbot` / `metadata.clawdis` / `metadata.openclaw`：ClawHub/OpenClaw 生态字段；这些块是增量运行时元数据，不是完整 YAML 头副本。
...
- `SKILL.md` 是标准文件名；v1 可接受小写 `skill.md` 作为生态互操作输入，但必须给 warning；若两者同时存在，优先使用 `SKILL.md` 并 warning。
...
至少保留 Agent 所需的标准元数据（`name`、`description`）与全部指令。`license`、`compatibility`、`metadata`、`allowed-tools` 等标准字段仍属于 `SKILL.md`。
...
3. **读取 `metadata.clawdbot` / `metadata.clawdis` / `metadata.openclaw`**。用于 ClawHub/OpenClaw 的运行需求声明；不读取并特殊处理 `metadata.clawhub`，除非上游定义该命名空间。
4. **读取可选 `skill.yaml`**。仅当 `metadata.skit` 不存在时读取；两者同时存在时报错，不进行合并；不得替代 `SKILL.md` 的标准字段。
此设计保持 `SKILL.md` 简洁，同时为新式 Skill 提供更清晰的包管理扩展路径。
...
**应对策略**：
- **分层降级**：`metadata.skit` / 外部同构 manifest / 生态运行时元数据 → 视为无 skit 依赖。
...
2. **生态字段读取**：规范依赖与系统需求优先读取 `metadata.skit` 和外部同构 manifest；ClawHub/OpenClaw 字段按 `metadata.clawdbot`、`metadata.clawdis`、`metadata.openclaw`、顶层 `clawdis`、顶层 fallback 字段顺序读取，用于诊断和展示，不执行其 `install` / `nix` / `config` 安装逻辑。
3. **手动模式默认**：`skit doctor` 打印清晰的安装建议，例如 `brew install ffmpeg`、`apt install ffmpeg`。
4. **交互修复可选**：后续可提供 `skit doctor --fix`，执行前必须明确确认。
5. **内置二进制谨慎引入**：对部分工具可拉取预编译二进制到 skit 管理目录，但必须校验来源 hash 和签名。
...
`skit install` 和 `skit search` 通过明确 provider 解析 source。当前默认支持 local、GitHub、GitLab 和 generic git；registry/well-known provider 在语义稳定后再引入配置。
...
skit v1 采用内容锁文件，并用 active symlink 表示当前安装状态：

- **项目 lock**：`.agents/skills/skit.lock`，与项目 active root 放在一起，不写入本机时间戳，字段排序稳定，目标是可复现和低冲突。
- **全局 lock**：`~/.agents/skills/skit.lock`，schema 与项目 lock 相同，用于记录全局激活的 Skill。
...
- `incomplete: true` 仅用于导入时无法形成可复现 restore 的条目；`skit install` 不自动恢复这类条目，而是提示用户使用明确 source 重新 `skit install` 或 `skit inspect` 查看原因。
- `warnings` 记录小写 `skill.md`、生态字段降级、导入信息丢失等非致命诊断。

### 5.5.3 生态导入策略

skit 可读取其他工具的 lock，但不默认双写其他工具的 lock：

- 若当前目录无 `.agents/skills/skit.lock`，但存在 `skills-lock.json`，`skit install` 可读取并恢复。
- 若当前目录无 `.agents/skills/skit.lock`，但存在 `.clawhub/lock.json`，`skit install` 可按 registry slug/version 恢复。
- `skit import-lock skills` 将 `skills-lock.json` 转为 `.agents/skills/skit.lock`。
- `skit import-lock clawhub` 将 `.clawhub/lock.json` 和 `.clawhub/origin.json` 转为 `.agents/skills/skit.lock`。
- 生态导入若无法还原 source/hash，必须生成 `incomplete: true` 条目并报告丢失字段。
...
skit v1 维护全局 canonical store、lockfile 和 active symlink。默认 project active root 为 `.agents/skills`，global active root 为 `~/.agents/skills`。
...
- **分发渠道开放**：v1 支持主流 GitHub/local/GitLab/generic git source；不把任何单一服务硬编码为唯一渠道。
...
- v1：快速、可复现。实现 Go 单二进制、主流 source syntax、project/global active root、全局 store、lockfile、local/GitHub/GitLab/generic git provider、doctor/inspect；registry search/download 作为增强能力。
